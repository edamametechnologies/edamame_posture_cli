...
def main() -> int:
    ...
    lines.append("")
    lines.append(
        "This report is regenerated by the `perf` job in"
        " `.github/workflows/tests.yml` on every push to `dev`/`main`"
        " (paths-filtered to `src/`, `Cargo.toml`, `Cargo.lock`) and on"
        " manual dispatch. It measures `edamame_posture` (built from the"
        " current checkout, or the last released binary when the dispatch"
        " toggle `build_from_source=false` is used) across six scenarios on"
    ...
    lines.append("```")
    lines.append("")
    lines.append("See the `perf` job in `.github/workflows/tests.yml` for the full CI orchestration.")
    lines.append("")
