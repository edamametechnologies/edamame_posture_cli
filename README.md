...
- name: Setup EDAMAME Posture
  run: |
    ...
    curl -LO https://github.com/edamametechnologies/edamame_posture_cli/releases/download/v0.9.22/edamame_posture-0.9.22-x86_64-unknown-linux-gnu
    chmod +x edamame_posture-0.9.22-x86_64-unknown-linux-gnu
    sudo mv edamame_posture-0.9.22-x86_64-unknown-linux-gnu /usr/local/bin/edamame_posture
...
After build and tests, verify that no unauthorized network connections occurred by examining the captured sessions against the whitelist:
- If any connection falls outside the whitelist, the command will exit non-zero and the pipeline should fail at this final step.
...
verify_network:
  stage: cleanup
  script:
    - edamame_posture get-sessions  # Exits non-zero if whitelist was violated
```
...
1. **Download** the Debian package for your platform:
   - **x86_64 (64-bit):** [edamame-posture_0.9.22-1_amd64.deb](https://github.com/edamametechnologies/edamame_posture_cli/releases/download/v0.9.22/edamame-posture_0.9.22-1_amd64.deb)
   - **i686 (32-bit):** [edamame-posture_0.9.22-1_i386.deb](https://github.com/edamametechnologies/edamame_posture_cli/releases/download/v0.9.22/edamame-posture_0.9.22-1_i386.deb)
   - **aarch64 (ARM 64-bit):** [edamame-posture_0.9.22-1_arm64.deb](https://github.com/edamametechnologies/edamame_posture_cli/releases/download/v0.9.22/edamame-posture_0.9.22-1_arm64.deb)
   - **armv7 (ARM 32-bit):** [edamame-posture_0.9.22-1_armhf.deb](https://github.com/edamametechnologies/edamame_posture_cli/releases/download/v0.9.22/edamame-posture_0.9.22-1_armhf.deb)
...
2. **Install** the package using either method:
   ```bash
   sudo apt install ./edamame-posture_0.9.22-1_amd64.deb
   # or
   sudo dpkg -i edamame-posture_0.9.22-1_amd64.deb
   ```
...
1. **Download Binary**: From the Releases page, download the binary for your architecture:
   - **x86_64 (64-bit)**: [edamame_posture-0.9.22-x86_64-unknown-linux-gnu](https://github.com/edamametechnologies/edamame_posture_cli/releases/download/v0.9.22/edamame_posture-0.9.22-x86_64-unknown-linux-gnu)  
   - **i686 (32-bit)**: [edamame_posture-0.9.22-i686-unknown-linux-gnu](https://github.com/edamametechnologies/edamame_posture_cli/releases/download/v0.9.22/edamame_posture-0.9.22-i686-unknown-linux-gnu)  
   - **aarch64 (ARM 64-bit)**: [edamame_posture-0.9.22-aarch64-unknown-linux-gnu](https://github.com/edamametechnologies/edamame_posture_cli/releases/download/v0.9.22/edamame_posture-0.9.22-aarch64-unknown-linux-gnu)  
   - **armv7 (ARM 32-bit)**: [edamame_posture-0.9.22-armv7-unknown-linux-gnueabihf](https://github.com/edamametechnologies/edamame_posture_cli/releases/download/v0.9.22/edamame_posture-0.9.22-armv7-unknown-linux-gnueabihf)
   - **x86_64 (64-bit) for Alpine Linux (musl)**: [edamame_posture-0.9.22-x86_64-unknown-linux-musl](https://github.com/edamametechnologies/edamame_posture_cli/releases/download/v0.9.22/edamame_posture-0.9.22-x86_64-unknown-linux-musl) 
   - **aarch64 (ARM 64-bit) for Alpine Linux (musl)**: [edamame_posture-0.9.22-aarch64-unknown-linux-musl](https://github.com/edamametechnologies/edamame_posture_cli/releases/download/v0.9.22/edamame_posture-0.9.22-aarch64-unknown-linux-musl)
...
1. **Download** the macOS universal binary:
   - [edamame_posture-0.9.22-universal-apple-darwin](https://github.com/edamametechnologies/edamame_posture_cli/releases/download/v0.9.22/edamame_posture-0.9.22-universal-apple-darwin)  
...
1. **Download** the Windows binary:
   - [edamame_posture-0.9.22-x86_64-pc-windows-msvc.exe](https://github.com/edamametechnologies/edamame_posture_cli/releases/download/v0.9.22/edamame_posture-0.9.22-x86_64-pc-windows-msvc.exe)
...
- **get-sessions**: Retrieve the captured network session data and check it against the whitelist. It will output a list of all observed network connections (with details) and return an exit code indicating whitelist conformance (0 if all connections were within the allowed whitelist, or 1 if any violation occurred). A special exit code 3 is used to indicate that no network sessions were recorded (for instance, if there was no network activity).
...
- **remediate-threat** `<THREAT_ID>` – Remediate a specific threat by its threat ID. *Requires admin privileges*. Returns exit code 1 if remediation fails.
- **rollback-threat** `<THREAT_ID>` – Roll back remediation for a specific threat by its threat ID. *Requires admin privileges*. Returns exit code 3 if invalid parameters.
...
- **check-policy** `<MINIMUM_SCORE>` `"<THREAT_IDS>"` `"[TAG_PREFIXES]"` – Local policy compliance check. *Requires admin privileges*. Returns exit code 1 if policy not met.
- **check-policy-for-domain** `<DOMAIN>` `<POLICY_NAME>` – Policy check against a Hub-defined domain policy. *Requires admin privileges*. Returns exit code 1 if policy not met.
- **check-policy-for-domain-with-signature** `"<SIGNATURE>"` `<DOMAIN>` `<POLICY_NAME>` – Verify a stored posture signature against a domain policy (for historical verification). Returns exit code 3 if invalid parameters.
...
- **request-pin** `<USER>` `<DOMAIN>` – Request a PIN for domain connection. Returns exit code 3 for invalid parameters.
...
- **get-sessions**: Returns 0 if session data was retrieved and all network connections conformed to the whitelist. Returns a non-zero code if a whitelist violation occurred (meaning unauthorized network activity was detected). Specifically, an exit code 1 indicates that sessions were found but some connection failed the whitelist check, and an exit code 3 indicates that no active network sessions were found (which is not an error, but a special case).
