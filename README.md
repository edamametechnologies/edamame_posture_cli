...
```
edamame_posture start --user <USER> --domain <DOMAIN> --pin <PIN> [--device-id <DEVICE_ID>] [--network-scan] [--packet-capture] [--whitelist <NAME>] [--fail-on-whitelist] [--fail-on-blacklist] [--fail-on-findings] [--include-local-traffic] [--cancel-on-violation] [--agentic-mode MODE] [--agentic-provider PROVIDER] [--agentic-interval SECONDS] [--export-to-portal]
```
...
(In practice, `--device-id`, `--network-scan`, `--packet-capture`, and `--whitelist` should be provided as needed for your setup. Additional flags such as `--fail-on-findings` or `--include-local-traffic` can further tailor behaviour.) This provides the most comprehensive CI/CD security controls:
...
```
edamame_posture background-start-disconnected [--network-scan] [--packet-capture] [--whitelist <NAME>] [--fail-on-whitelist] [--fail-on-blacklist] [--fail-on-findings] [--include-local-traffic] [--cancel-on-violation] [--llm-api-key <KEY>] [--agentic-mode MODE] [--agentic-provider PROVIDER] [--agentic-interval SECONDS]
```
...
1. **Download** the Debian package for your platform:
   - **x86_64 (64-bit):** [edamame-posture_1.3.13-1_amd64.deb](https://github.com/edamametechnologies/edamame_posture_cli/releases/download/v1.3.13/edamame-posture_1.3.13-1_amd64.deb)
   - **i686 (32-bit):** [edamame-posture_1.3.13-1_i386.deb](https://github.com/edamametechnologies/edamame_posture_cli/releases/download/v1.3.13/edamame-posture_1.3.13-1_i386.deb)
   - **aarch64 (ARM 64-bit):** [edamame-posture_1.3.13-1_arm64.deb](https://github.com/edamametechnologies/edamame_posture_cli/releases/download/v1.3.13/edamame-posture_1.3.13-1_arm64.deb)
     - For Raspberry Pi 3/4/5 running 64-bit OS
   - **armv7 (ARM 32-bit):** [edamame-posture_1.3.13-1_armhf.deb](https://github.com/edamametechnologies/edamame_posture_cli/releases/download/v1.3.13/edamame-posture_1.3.13-1_armhf.deb)
...
2. **Install** the package using either method:
   ```bash
   sudo apt install ./edamame-posture_1.3.13-1_amd64.deb
   # or
   sudo dpkg -i edamame-posture_1.3.13-1_amd64.deb
   ```
...
1. **Download Binary**: From the Releases page, download the binary for your architecture:
   - **x86_64 (64-bit)**: [edamame_posture-1.3.13-x86_64-unknown-linux-gnu](https://github.com/edamametechnologies/edamame_posture_cli/releases/download/v1.3.13/edamame_posture-1.3.13-x86_64-unknown-linux-gnu)
   - **i686 (32-bit)**: [edamame_posture-1.3.13-i686-unknown-linux-gnu](https://github.com/edamametechnologies/edamame_posture_cli/releases/download/v1.3.13/edamame_posture-1.3.13-i686-unknown-linux-gnu)
   - **aarch64 (ARM 64-bit)**: [edamame_posture-1.3.13-aarch64-unknown-linux-gnu](https://github.com/edamametechnologies/edamame_posture_cli/releases/download/v1.3.13/edamame_posture-1.3.13-aarch64-unknown-linux-gnu)
   - **armv7 (ARM 32-bit)**: [edamame_posture-1.3.13-armv7-unknown-linux-gnueabihf](https://github.com/edamametechnologies/edamame_posture_cli/releases/download/v1.3.13/edamame_posture-1.3.13-armv7-unknown-linux-gnueabihf)
   - **x86_64 (64-bit) for Alpine Linux (musl)**: [edamame_posture-1.3.13-x86_64-unknown-linux-musl](https://github.com/edamametechnologies/edamame_posture_cli/releases/download/v1.3.13/edamame_posture-1.3.13-x86_64-unknown-linux-musl)
   - **aarch64 (ARM 64-bit) for Alpine Linux (musl)**: [edamame_posture-1.3.13-aarch64-unknown-linux-musl](https://github.com/edamametechnologies/edamame_posture_cli/releases/download/v1.3.13/edamame_posture-1.3.13-aarch64-unknown-linux-musl)
...
```bash
chmod +x edamame_posture-1.3.13-x86_64-unknown-linux-gnu
sudo mv edamame_posture-1.3.13-x86_64-unknown-linux-gnu /usr/local/bin/edamame_posture
```
...
1. **Download** the macOS universal binary:
   - [edamame_posture-1.3.13-universal-apple-darwin](https://github.com/edamametechnologies/edamame_posture_cli/releases/download/v1.3.13/edamame_posture-1.3.13-universal-apple-darwin)
...
1. **Download** the Windows binary:
   - [edamame_posture-1.3.13-x86_64-pc-windows-msvc.exe](https://github.com/edamametechnologies/edamame_posture_cli/releases/download/v1.3.13/edamame_posture-1.3.13-x86_64-pc-windows-msvc.exe)
...
- **background-vulnerability-status** / **vulnerability-status** `[--fail-on-findings]`: Display runtime vulnerability detector status. Use `--fail-on-findings` in CI/CD to return a non-zero exit code when active runtime vulnerability findings exist. The gate consumes `active_alertable_findings` (HIGH/CRITICAL severity only) so LOW-severity ambient findings (e.g. CI bootstrappers running from `/tmp/`, build scripts writing benign `.log` artifacts) stay visible in the dashboard without by themselves failing the run. Older daemons that predate this counter fall back to the raw `active_findings` total. The gate does not require an LLM, but an LLM is recommended for CI/security use because adjudication and suppression reduce noise and improve alert text.
...
- **background-vulnerability-status** (alias **vulnerability-status**) `[--fail-on-findings]` – Display runtime vulnerability detector status. With `--fail-on-findings`, returns non-zero when `active_alertable_findings` (HIGH/CRITICAL severity, non-dismissed) is greater than zero, making it suitable as a CI/CD stop-on-vulnerability gate. LOW-severity findings (ambient lineage from CI bootstrappers, benign temp `.log`/`.txt` writes, etc.) still appear in `active_findings` for dashboard visibility but do not by themselves trip the gate. Older daemons that predate the alertable counter fall back to `active_findings`. No LLM is required for raw findings, but LLM configuration is recommended for adjudication, suppression, and clearer alerts.
...
jobs:
  verify-security-signature:
    ...
    steps:
      ...
      - name: Install EDAMAME Posture
        run: |
          curl -LO https://github.com/edamametechnologies/edamame_posture_cli/releases/download/v1.3.13/edamame_posture-1.3.13-x86_64-unknown-linux-gnu
          chmod +x edamame_posture-1.3.13-x86_64-unknown-linux-gnu
          sudo mv edamame_posture-1.3.13-x86_64-unknown-linux-gnu /usr/local/bin/edamame_posture
